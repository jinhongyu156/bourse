...
const styles = StyleSheet.create( {
	...
	headerLeftViewInfo: { flex: 1, height: HEADERLOGOSIZE, justifyContent: "center", marginLeft: 10 },
	headerLeftViewInfoText: { fontSize: 12, paddingVertical: 2, color: "#FFFFFF" }

} );

export default React.memo( function Header( { children, logoKey, simulatorUsdtInfo, usdtInfo } )
{
	return <ImageBackground source = { require( "./../images/header.png" ) } style = { styles.headerBackground }>
		<View style = { styles.headerLeftView }>
			...
			<View style = { styles.headerLeftViewInfo }>
				<Text style = { styles.headerLeftViewInfoText }>{ I18n.t( "header.simulatorUsdt" ) }: { simulatorUsdtInfo }</Text>
			</View>
		</View>
