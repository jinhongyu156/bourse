...
const styles = StyleSheet.create( {
	...
	borderText: { flex: 3, fontSize: 12, paddingVertical: 6, borderBottomWidth: 1, borderColor: "#8B8B9A", textAlign: "center" },
	...
	} else
	{
		if( Number( data[ "投资ETUSD" ] ) > 0 )
		{
			return <ScrollView style = { styles.container } contentContainerStyle = { styles.contentContainer } showsVerticalScrollIndicator = { false }>
				...
				<Text style = { styles.text }>&emsp;&emsp;1.甲方自愿投资比特币矿机 { ( Number( data[ "矿机算力" ] ) / 53 ).toFixed( 3 ) } 台, 型号 53T, 单价 3000USDT/台.此款在合同签订之日起 10 日内交付, 由甲方负责采购并托管给乙方运营, 乙方需将甲方投资的机器放置在条件有利于双方互利互惠的场地运营, 矿机的产权归甲方所有.</Text>
				...
				<Text style = { styles.text }>&emsp;&emsp;合同有效期自{ data[ "年" ] }年{ data[ "月" ] }月{ data[ "日" ] }起至{ data[ "至年" ] }年{ data[ "至月" ] }月{ data[ "至日" ] }日止(最低不低于一年), 如未填写按甲方在乙方平台确认时间为准自动顺延</Text>
				...
				<Text style = { styles.text }>&emsp;&emsp;1.投资期限到期按投资当日生效起, 最低需满一年, 如甲方在一年内退出投资, 甲方需向乙方支付矿机运营附属设施费用, 该费用按甲方投资金额的 20% 进行计算.</Text>
				<Text style = { styles.text }>&emsp;&emsp;1.投资期限到期按投资当日生效起, 最低需满一年.</Text>
				...
				<Text style = { styles.text }>四、交易金规定: </Text>
				...
				<Text style = { styles.text }>五、违约处理: </Text>
				<Text style = { styles.text }>&emsp;&emsp;1.甲方签订合同时应全额付清购机款, 否则乙方不予退还所收定金, 定金交易模式(预付金)</Text>
				{/* <Text style = { styles.text }>&emsp;&emsp;2.甲方达到回购条件时, 如因乙方不及时回购致甲方投资超期, 除正常支付分红金额外, 另需支付一倍的分红金额作为违约金, 但最长超期不得超过一年.</Text> */}
				<Text style = { styles.text }>六、其他约定: </Text>
				...
				<Text style = { styles.text }>七、解决合同纠纷的方式: </Text>
				<Text style = { styles.text }>&emsp;&emsp;本合同发生争议, 由甲乙双方协商解决, 协商不成, 由公司所在地或江油市仲裁委员会仲裁.</Text>
				<Text style = { styles.text }>八、本合同一式三份, 甲方一份、乙方两份, 具有同等法律效力.</Text>
				<Text style = { styles.text }>九、本合同经双方签字盖章之日起生效.</Text>
				<Text style = { styles.text }>十、本合同内容我已认真阅读并确认.同意合同内容.并自愿承担由此产生的经济责任.</Text>
